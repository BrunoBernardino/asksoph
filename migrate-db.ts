...
async function getExecutedMigrations() {
  const executedMigrations = new Set(
    Array.from(
      (await db.query<Migration>(sql`SELECT * FROM "asksoph_migrations" ORDER BY "name" ASC`)).map((migration) =>
        migration.name
      ),
    ),
  );
...
async function runMigrations(missingMigrations: Set<string>) {
  for (const missingMigration of missingMigrations) {
    ...
    try {
      ...
      await db.query(sql`INSERT INTO "public"."asksoph_migrations" ("name", "executed_at") VALUES ($1, NOW())`, [
        missingMigration,
      ]);
